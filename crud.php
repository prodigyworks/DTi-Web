...
class Crud  {
	...
	public function triggerRefresh() {
		if (count($this->errorDescriptions) > 0) {
			echo "<html><body><script>window.parent.showError('" . escape_notes($this->errorDescriptions[0]) . "');</script></body></html>";
			
		} else {
			echo "<html><body><script>window.parent.refreshData();</script></body></html>";
		}
	}
...
<?php
		foreach ($this->columns as $col) {
			if ($col['filter']) {
				...
				if ($col['type'] == "TEXTBOX") {
					if ($col['datatype'] == "timestamp" || $col['datatype'] == "datetime") {
						echo "<input class='datepicker' type='text' id='filter_" . $col['name'] . "' name='filter_" . $col['name'] . "'  />\n";
						
					} else if ($col['datatype'] == "date") {
						echo "<input class='datepicker' type='text' id='filter_" . $col['name'] . "' name='filter_" . $col['name'] . "' />\n";
						
					} else if ($col['datatype'] == "time") {
						echo "<input class='timepicker' type='text' id='filter_" . $col['name'] . "' name='filter_" . $col['name'] . "' />\n";
					...
					echo "<div  style='width:600px; height: 150px; overflow:auto' id='filter_" . $col['name'] . "' name='filter_" . $col['name'] . "'></div>\n";
...
//			}
			...
			if ($okToRun) {
				if ($app['application'] != "") {
				?>
				   	<span title="<?php echo $app['tooltip']; ?>" id="<?php echo $app['id']; ?>"  class="wrapper"><a class='rgap2 link2' href="javascript:application('<?php echo $app['application']; ?>')"><em><b><img src='<?php echo $app['imageurl']; ?>' /> <?php echo $app['title']; ?></b></em></a></span>
				<?php
					
				} else {
				?>
				   	<span  title="<?php echo $app['tooltip']; ?>" id="<?php echo $app['id']; ?>"  class="wrapper"><a class='rgap2 link2' href="javascript:<?php echo $app['script']; ?>()"><em><b><img src='<?php echo $app['imageurl']; ?>' /> <?php echo $app['title']; ?></b></em></a></span>
				<?php
				}
			}
		}
		...
		?> 
	   	<span class="wrapper"><a class='rgap2 link2' href="javascript:addCrudItem()"><em><b><img src='images/add.png' /> Add</b></em></a></span>
		<?php
				}
			}
		?>
...
<?php
		...
		<?php
			if (isset($_GET['puri'])) {
		?>
		   	<span class="rgap5 wrapper"><a class='rgap2 link2' href="javascript:back()"><em><b><img src='images/back2.png' /> Back</b></em></a></span>
...
?>									
					...
					$(".entryform select[required], .entryform input[required]").each(
							function() {
...
?>
	...
	public function update($id) {
		try {
			...
			if (! $result) {
				if (mysql_errno() == 1062) {
					$this->errorDescriptions[] = "Duplicate value. Row already exists";
					
				} else {
					$this->errorDescriptions[] = mysql_error();
				}
	...
	public function insert() {
		try {
			...
			if (! $result) {
				...
				$this->postInsertEvent(mysql_insert_id());
			}
